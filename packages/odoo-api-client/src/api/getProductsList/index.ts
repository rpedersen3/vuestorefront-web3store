...
import query from './getProductListQuery';
...
export default async function getProductsList(
  ...
  const { getProductsList } = context.extendQuery(
    customQuery, { getProductsList: { query, variables: params } }
  );

  const response = await apolloClient.query({
    query: gql`${getProductsList.query}`,
    variables: getProductsList.variables,
    errorPolicy: 'all'
  });
