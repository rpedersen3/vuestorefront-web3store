...
import query from './getProductAssetsListQuery';
import { GraphQlGetProductParams, ProductResult, ProductAssetResult } from '../../index';
...
export default async function getProductVariantsList(
  ...
  const { getProductAssetsList } = context.extendQuery(
    customQuery, { getProductAssetsList: { query, variables: params } }
  );

  const response = await apolloClient.query({
    query: gql`${getProductAssetsList.query}`,
    variables: getProductAssetsList.variables,
    errorPolicy: 'all'
  });
  ...
  if (cacheKey &&
    ...
    redisClient.set(
      cacheKey,
      response,
      [`API-C${categoryIdForCache}-productassets`],
