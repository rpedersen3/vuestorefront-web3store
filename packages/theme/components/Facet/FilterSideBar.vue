      ...
      <div v-for="(facet, i) in facets" :key="i">
        <template v-if="isFacetPrice(facet)">
          ...
          <SfRange
            :value="[20, 600]"
            :disabled="false"
            :config="config" v-model="price" key="price"
            @change="selectPrice"
          />
        </template>
        ...
        <template>
          ...
          <SfRange
            :disabled="false"
            :config="facet.config"  @change="(values) => selectRange(values, facet)" 
          />
          </div>
          <div
            v-else-if="isFacetColor(facet, facet.options)"
            class="filters__colors"
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const { changeFilters, isFacetColor, isFacetRange, isFacetPrice, facetsFromUrlToFilter } =
      useUiHelpers();
    ...
    const selectRange = (values, facet) => {
      ...
      const selectedValue = selectedFilters.value.find(
        (item) => item?.filterName === facet.name
      );
    ...
    const selectFilter = (facet, option) => {
      ...
      console.info("(Rich) select filter: ")
